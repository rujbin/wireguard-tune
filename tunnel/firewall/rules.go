...
// addFilter is a helper function to create displayData, set the layer key and add the filter.
func addFilter(session uintptr, filter *wtFwpmFilter0, layerKey windows.GUID, displayMessage string) error {
	displayData, err := createWtFwpmDisplayData0(displayMessage, "")
	if err != nil {
		return wrapErr(err)
	}
...
func permitTunInterface(session uintptr, baseObjects *baseObjects, weight uint8, ifLUID uint64) error {
	...
	// Verarbeite alle vier Filter in einer Schleife.
	filters := []struct {
		layer windows.GUID
		msg   string
	}{
		{cFWPM_LAYER_ALE_AUTH_CONNECT_V4, "Permit outbound IPv4 traffic on TUN"},
		{cFWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V4, "Permit inbound IPv4 traffic on TUN"},
		{cFWPM_LAYER_ALE_AUTH_CONNECT_V6, "Permit outbound IPv6 traffic on TUN"},
		{cFWPM_LAYER_ALE_AUTH_RECV_ACCEPT_V6, "Permit inbound IPv6 traffic on TUN"},
	}

	for _, f := range filters {
		if err := addFilter(session, &filter, f.layer, f.msg); err != nil {
			return err
		}
	}
